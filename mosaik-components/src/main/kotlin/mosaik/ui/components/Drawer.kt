...
/**
 * A label-based drawer trigger targeting the checkbox generated by [mDrawer].
 *
 * It reuses the same modifier vocabulary as [mButton] while rendering a label
 * with `for` wired to [toggleId], matching DaisyUI's CSS-only drawer contract.
 */
fun FlowContent.mDrawerButton(
    toggleId: String,
    ...
    label(
        classes = buttonClasses(variant, style, shape, width, size, buildClasses("drawer-button", classes)),
    ) {
        htmlFor = toggleId
        block()
    }
