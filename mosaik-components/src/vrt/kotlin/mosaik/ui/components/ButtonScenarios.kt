...
object ButtonScenarios {
    ...
    private fun styleScenario(style: ButtonStyle) =
        Scenario("button-style-${style.token}") {
            mButton(variant = ButtonVariant.Primary, style = style) { +"Button" }
        }

    private fun shapeScenario(shape: ButtonShape) =
        Scenario("button-shape-${shape.token}") {
            mButton(variant = ButtonVariant.Primary, shape = shape) { +"Icon" }
        }

    private fun widthScenario(width: ButtonWidth) =
        Scenario("button-width-${width.token}") {
            mButton(variant = ButtonVariant.Primary, width = width) { +"Button" }
        }

    private fun sizeScenario(size: Size) =
        Scenario("button-size-${size.token ?: "md"}") {
